...
#include "../hash/uint1024.h" // for uint64
...
public:
    ...
    int getint() const
    {
        ...
        if (!BN_is_negative(this))
			return (int)(n > (unsigned long)std::numeric_limits<int>::max() ? std::numeric_limits<int>::max() : n);
        else
            return (int)(n > (unsigned long)std::numeric_limits<int>::max() ? std::numeric_limits<int>::min() : -(int)n);
    }
